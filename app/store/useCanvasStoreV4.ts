...
export const useStore = createStore<CanvasState>((set, get) => ({
  ...
  setNodes: (updater) => {
    ...
    set(
      (state: {
        ...
        nodeInternals: {
          clear: () => void;
          set: (arg0: string, arg1: Node) => void;
        };
      }) => {
  ...
  addNode: async (node) => {
    ...
    if (error) {
      ...
      const textColor =
        createdNode.data && createdNode.data.backgroundColor
          ? parseInt(createdNode.data.backgroundColor.replace('#', ''), 16) >
            0xffffff / 2
            ? '#575757'
            : '#F4F4F4'
          : '#575757';
      ...
      const newNode = {
        ...
        style: {
          backgroundColor:
            (createdNode.data && createdNode.data.backgroundColor) || '#F4F4F4', // Default or specified background color
      ...
      switch (node.type) {
        case 'note':
          newNode.data = {
            ...newNode.data,
            ...(createdNode.data as TablesInsert<'note_nodes'>)
          };
          break;
        case 'task':
          newNode.data = {
            ...newNode.data,
            ...(createdNode.data as TablesInsert<'task_nodes'>)
          };
          break;
        case 'table':
          newNode.data = {
            ...newNode.data,
            ...(createdNode.data as TablesInsert<'table_nodes'>)
          };
          break;
        case 'calendar':
          newNode.data = {
            ...newNode.data,
            ...(createdNode.data as TablesInsert<'calendar_nodes'>)
          };
          break;
        case 'draw':
          newNode.data = {
            ...newNode.data,
            ...(createdNode.data as TablesInsert<'draw_nodes'>)
          };
          break;
  ...
  addEdge: async (edge) => {
    ...
    const { canvasID } = get();
    const { data: createdEdge, error } = await createEdgeInDatabase({
    ...
    if (error) {
      ...
      set((state) => ({
        edges: [...state.edges, { ...edge, id: createdEdge.id }]
      }));
    }
  },
  removeNode: (id) => {
  ...
  fetchCanvas: async (canvasId: string) => {
    console.log('Store: Fetching canvas with ID:', canvasId);
    try {
      const result = await fetchCanvas(canvasId);
      if (
        result.data &&
        result.nodeData &&
        ...
        set(() => ({ nodes, edges, canvasID: canvasId }));
      } else if (result.error) {
        console.error('Error fetching canvas:', result.error);
      } else {
        console.error('Unexpected result structure from fetchCanvas');
      }
    } catch (error) {
