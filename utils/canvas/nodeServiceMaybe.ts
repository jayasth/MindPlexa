...
export const updateNode = async (
  ...
  const { data, error } = await supabase.rpc('update_node_transaction', {
    p_node_id: nodeId,
    p_node_type: nodeType,
    p_node_data: nodeData ? toSnakeCase(nodeData) : null,
    p_specific_node_data:
      Object.keys(specificNodeData).length > 0
        ? toSnakeCase(specificNodeData[`${nodeType}Data`])
        : null,
    p_tags: tags || [],
    p_attachments: (attachments as []) || []
  });
  ...
  console.log('nodeService: Node updated successfully:', data);
