...
import { getSvgString } from './utils/svgUtils';
...
const DrawNodeEdit: React.FC<DrawNodeEditProps> = ({
  ...
  const handleDrawingChange = useCallback((newDrawingData: string) => {
    // Convert the canvas content to SVG
    const svgString = getSvgString(artboardRef.current?.canvas);
    setDrawingData(svgString);
  }, []);
